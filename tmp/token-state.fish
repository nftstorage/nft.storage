...
curl -s 'localhost:8787/internal/tokens' -H "authorization: Bearer $API_KEY" -H 'content-type: application/json' --data-raw '{"name":"abc"}' | jq

curl -s 'localhost:8787/internal/tokens' -H "authorization: Bearer $API_KEY" -H 'content-type: application/json' | jq '.value | length'