...
As time goes on, NFT.Storage's ability UCANs can be expanded (e.g., blacklisting CIDs from being uploaded, permission to update mutable references). You can read more about how to implement UCAN-signed uploads in [our docs]() or on our UCAN implementation's [README](https://github.com/nftstorage/ucan.storage).
