...
NFT storage of off-chain assets is a tricky thing. There are some key requirements: the storage needs to be content-addressed and multi-generational at the very least. Ideally, it’s on decentralized storage so as the NFT gets bought and sold, the risk that the off-chain data is lost is minimal. These are all table stakes things that anyone minting or buying an NFT should double check.
