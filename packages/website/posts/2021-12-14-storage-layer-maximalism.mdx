...
NFTs continue to grow mainstream adoption, and as we push into new frontiers (Videos, GameFi, Metaverse) it’s critical that we take a keen eye to ensuring digital assets are created in a way ready to stand the test of time. In this post, we’ll make the case for ***storage layer maximalism***, and why every NFT needs a CID.

## What exactly is an NFT?


**If you’re familiar with what's happening under the hood, skip this section - this is just to get everyone up to speed.**

If you ask the average person what an NFT is, they’ll probably tell you it's a picture (or video or something) that “lives on the blockchain”. Because it “lives on the blockchain”, you get all these cool properties (Provenance! Automatic royalties! User ownership!) - which has birthed a whole movement in digital creation.

But if you dig in a bit, you’ll quickly realize that there’s a bit more nuance in both how an NFT is structured, and how your data is being stored. While the NFT (the actual token) lives on the blockchain - all the stuff (the information that shows up on NFT marketplaces, the image, other associated files - all commonly referred to as the metadata) associated with the NFT is usually referenced by external links[^1*].

![](https://user-images.githubusercontent.com/11778450/145664635-22e0e3d6-7207-47aa-8cee-bb94cd4a1eac.png | width=100)![](https://user-images.githubusercontent.com/11778450/145664649-69351fdc-3aac-4222-bb16-59b8a8fed4cd.png | width=100)
<p align="center">
**On the left is the ERC-721 spec for NFTs. The TokenURI field contains a link to your metadata blob (seen on the right), which contains a link to your image (highlighted).**
</p>
...
As anyone who has been on the internet for a bit is aware, the web is a (surprisingly) fragile place:
- Links can point at dead content (404s)
- Domains can be sold, and the same link resolves to some unexpected new content
...
1. How do we make sure the link our NFT uses actually points at the thing we want it to represent (aka my NFT is linked to its content, not someone else’s server)?
2. How do we make sure the link our NFT uses always resolves to the content we expect it to (aka prevent our jpegs from disappearing)?


## Solving the linking problem

For the first challenge, we need to rethink how we use links. Traditionally, when we talk about links we’re normally talking about HTTP URLs - the type of URL that’s likely sitting in your browser as you read this. 
...
A simple analogy might help make this clearer:
Imagine we were talking about “To Kill A Mockingbird” and I wanted to describe how you could get a copy of it. In the way that HTTP URLs work, sharing this book with you would be like describing to you where you could find it (“Come to NYC, at these cross streets you’ll find a library, 3rd bookshelf in - 2 books from the right is the one you want). Naturally, many things can go wrong (you could get blocked from coming to NYC, the library could burn down, a different book could be in the spot I mentioned, etc). 
...
![](https://user-images.githubusercontent.com/11778450/145664668-b3b4ed13-641d-4143-a312-11fd3198ae32.png | width=100)
<p align="center">
...
- Tezos: [FA-2](https://assets.tqtezos.com/docs/token-contracts/fa2/2-fa2-nft-tutorial/)

By and large this is done because storing data on-chain is really expensive.

[^2]: Should note there are some exceptions to this - like storing SVGs directly on chain, but this is a pretty small subset of what NFTs (hopefully) will end up enabling.
