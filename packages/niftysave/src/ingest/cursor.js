 ...
 * @returns { Promise<number>}
 */
export async function initIngestionQueueCursor(config) {
  let initDate = new Date().toDateString() //default is to start today
  ...
  const lastImport = await Hasura.query(config.hasura, {
   ...
   * ERC721 is in *seconds* JS is in *ms* so /1000
   */
  let mint_time = lastImport?.erc721_token_ingestion_queue[0]?.mint_time || 0
