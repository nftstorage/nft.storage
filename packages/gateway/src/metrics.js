...
export async function metricsGet(request, env, ctx) {
  ...
  const metrics = [
    ...
    `nftgateway_summary_responses_total{env="${env.ENV}"} ${metricsCollected
      .summaryMetrics.totalWinnerSuccessfulRequests +
    ...
    `nftgateway_summary_responses_per_time_total{le="+Inf",env="${
      env.ENV
    ...
    ...env.ipfsGateways.map(
      gw =>
        `nftgateway_failed_requests_total{gateway="${gw}",env="${
          env.ENV
          ...
          .map(
            status =>
              `nftgateway_requests_by_status_total{gateway="${gw}",env="${
                env.ENV
              }",status="${status}"} ${metricsCollected.ipfsGateways[gw]
                .totalResponsesByStatus[status] || 0}`
          )
          .join('\n')
      })
      .filter(e => !!e),
          ...
          .map(
            reason =>
              `nftgateway_prevented_requests_by_reason_total{gateway="${gw}",env="${
                env.ENV
              }",reason="${reason}"} ${metricsCollected.ipfsGateways[gw]
    ...
    `nftgateway_responses_content_length_total{le="+Inf",env="${
      env.ENV
    }"} ${metricsCollected.summaryMetrics.totalWinnerSuccessfulRequests +
      metricsCollected.summaryMetrics.totalCachedResponses}`,
