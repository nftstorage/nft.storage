...
CREATE TABLE IF NOT EXISTS upload (
    ...
    - - 1.Pinning Service API user provided `Record<string, string>`.-- 2. Metaplex endpoint `/metaplex/upload` to store details of the Metaplex user.
    meta jsonb,
    inserted_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    deleted_at TIMESTAMP WITH TIME ZONE,
...
from public.user u
    ...
    full outer join (
        ...
        where deleted_at is null
    ) as akh on akh.auth_key_id = ak.id
where ak.deleted_at is NULL
    or ak.deleted_at is not NULL
    and akh.status is not NULL;
